...
class ShoesDataset(Dataset):
    ...
    def __init__(self, path_a, transform, output_res=256, train=True, perc=1):
        self.train = train
    ...
    def transform_set(self, image_a, image_b):
        # Apply augmentation
        trans_dict = {'image': image_a, 'image_b': image_b}
...
def data_load(path_a, transform, batch_size, shuffle=False, output_res=128, train=True, perc=1):
    # Wrapper for loader
    dataset = ShoesDataset(path_a, transform, output_res=output_res, train=train, perc=perc)
